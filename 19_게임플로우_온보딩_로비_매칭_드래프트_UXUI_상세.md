...
## 19) UI 구현 명세 연동 기준 (21 문서 참조)

`21_UXUI_화면별_컴포넌트_상세명세서.md`를 본 문서의 **구현 레벨 상세본**으로 지정한다.

적용 원칙:
1. 상태 전이 정의는 19가 소스 오브 트루스
2. 화면 컴포넌트/상태 바인딩/이벤트 바인딩/A11y/QA는 21이 소스 오브 트루스
3. 서버 이벤트 필드/오류코드는 20과 1:1 매핑

릴리스 게이트(필수):
- [ ] AppFlowState 12개 상태가 21의 화면 루트와 모두 연결되어 있는가
- [ ] `C2S_*` 요청별 로딩 잠금/중복 방지 처리 완료
- [ ] `S2C_*` 실패 코드별 복구 UI(토스트/모달/CTA) 존재
- [ ] 접근성 공통 규칙(포커스, 라이브리전, 색상 대비) 충족
- [ ] QA 시나리오(정상/예외/회귀) 통과

운영 메모:
- 신규 화면/모달이 추가되면 19의 상태 전이와 21의 컴포넌트 명세를 동시에 업데이트한다.
- 문서 간 충돌 시 우선순위는 `19(흐름)` -> `20(프로토콜)` -> `21(구현 UI)` 순으로 조정한다.
