...
헷갈림 방지를 위해 용어를 먼저 고정한다.

- **Room(룸 인스턴스)**: 실제 한 판(매치)이 돌아가는 시뮬레이션 단위
- **Room Server Process(룸 서버 프로세스)**: `wildpaw-room` 실행 프로세스 1개
- **Server Host(서버 머신/인스턴스)**: 프로세스를 올리는 물리/가상 서버

현재 M1 구현은 다음 의미다.

- **룸 서버 프로세스 1개 = 룸 인스턴스 1개**
- 하지만 **서버 머신 1대 = 룸 1개**를 뜻하지는 않는다.
  - 같은 서버 머신에서 `wildpaw-room` 프로세스를 여러 개 띄우면 룸도 여러 개가 된다.

추가로, 아래 두 개념은 별개다.

- **룸 개수(rooms)**: 현재 떠 있는 룸 인스턴스 수
- **룸 정원(capacity)**: 룸 1개에 들어갈 수 있는 플레이어 수 (예: 3:3=6명, 5:5=10명)
...
1. 지금 동접이 몇 명인가?
2. 지금 “방(룸 인스턴스)”이 몇 개인가?
   - M1: 프로세스당 룸 1개 모델이므로 이 프로세스 기준 `rooms=1`
   - M2: 여러 룸 프로세스를 집계하여 `rooms=N` 표시
   - 주의: `rooms`는 룸 개수이며, 3:3/5:5 같은 팀 규칙(룸 정원)과는 다른 값이다.
3. 팀 슬롯이 어떻게 배정되었는가? (team1/team2, slot)
4. 누가 접속해 있는가? (playerId, 연결 시간, 마지막 패킷 시각)
5. 접속한 사람의 IP/포트 정보는 무엇인가?
6. 정원 초과 접속이 차단되는가? (`room.full` 응답 후 종료)
7. 비정상 패킷을 보내는 사람은 없는가?
   - invalid FlatBuffers envelope
   - text frame(C2S text)
   - unsupported message type
   - 과도한 입력/패킷 rate
   - profile invalid 요청 반복 등
8. 서버 틱이 정상인가? (tick overrun, last tick duration)
9. retransmit이 과도하게 발생하는가? (sent/dropped/inflight)
10. 룰(combat_rules.json)을 바꿨을 때 서버가 반영했는가? (hot reload)
...
- 서버 정보
  ...
  - uptime(선택)
- 룸/동접
  - rooms (룸 인스턴스 수, M1 현재값=1)
  - teamSize / maxPlayersPerRoom
  - team occupancy(team1/team2)
...
- `GET /admin/api/status`
  - `teamSize`, `maxPlayersPerRoom`, `teamOccupancy` 포함
- `GET /admin/api/sessions`
  - 세션별 `teamId`, `teamSlot`, remote/ip, 카운터 포함
